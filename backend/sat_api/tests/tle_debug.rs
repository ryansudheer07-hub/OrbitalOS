...
async fn main() {
    ...
    // Test direct URL fetch to see raw data
    println!("\n🔍 Testing direct data fetch from active.txt...");
    let client = reqwest::Client::new();
    match client
        .get("https://celestrak.org/NORAD/elements/active.txt")
        .send()
        .await
    {
        Ok(response) => {
            ...
            match response.text().await {
                Ok(text) => {
                    ...
                    println!("📄 First 20 lines of active.txt:");
